...
define("config/Meta", ["./products", "./main/main", "./Ruleset"], function (productsModule, mainModule, Ruleset) {
    ...
    for (const product of meta.products) {
        if (meta.productsById[product.id]) throw new Error(`Purchase with id ${product.id} already exists!`);
        if (meta.productsByIdNum[product.idNum]) throw new Error(`Purchase with idNum ${product.idNum} already exists!`);
